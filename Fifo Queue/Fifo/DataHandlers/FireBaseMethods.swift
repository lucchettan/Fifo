...
/* To load the data on a view, add :
 ...
 .onAppear {
     ...
     FireBaseMethods().getTicketsFromAQueueName(queueName: String { (result) in
         switch result {
             case .success(let theResult):
                 self.myticket = theResult
...
struct FireBaseMethods {
                            ...
                            if error != nil {
                                ...
                                if snapshot?.isEmpty != true && snapshot != nil {
                                    ...
                                    for document in snapshot!.documents {
                                        ...
                                        var queue = Queue(queueID: queueID, name: name, email: email)
    ...
    /* --------CREATE A TICKET--------- */
    func createTicket(queue: Queue, token: String){
            //instanciate our DB
    ...
    /* -----We create key pair values:  [field : content, postedBy : Auth.auth().currentUser! etc etc]
            ...
            let firePost : [String: Any] = ["queueName" : queue.name, "token" : token]
...
/* -------- We create key pair values:  [field : content, postedBy : Auth.auth().currentUser! etc etc]
              This way we define our object into our DB with the arguments
              and we cast it as [string: any] to fit with firestore compliance   */
            let firePost : [String: Any] = ["queueName" : queue.name, "email" : Auth.auth().currentUser!.email!]
